<script lang="ts">
	...
	import { newEquipmentSchema, type NewEquipmentSchema } from "$lib/zod-schemas";
	...
	export let theForm: SuperValidated<Infer<NewEquipmentSchema>>;
	...
	const form = superForm(theForm, {
		validators: zodClient(newEquipmentSchema),
		onUpdate(event) {
			if (!event.form.valid) {
				return;
			}
...
<form
	action="/admin/inventory/new/?/newEquipment"
	method="POST"
	use:enhance
