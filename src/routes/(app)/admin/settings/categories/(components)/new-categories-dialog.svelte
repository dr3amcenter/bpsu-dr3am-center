<script lang="ts">
	...
	import { newCategoriesSchema, type NewCategoriesSchema } from "../(data)/schema.js";
	...
	export let theForm: SuperValidated<Infer<NewCategoriesSchema>>;

	let open = false;

	const form = superForm(theForm, {
		validators: zodClient(newCategoriesSchema),
		onUpdate(event) {
			if (event.form.valid) {
				open = false;
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="max-h-[90vh] overflow-auto">
		...
		<form use:enhance method="POST" action="?/newCategories">
