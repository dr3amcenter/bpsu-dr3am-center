<script lang="ts">
	...
	import { newLocationsSchema, type NewLocationsSchema } from "../(data)/schema.js";
	...
	export let theForm: SuperValidated<Infer<NewLocationsSchema>>;

	let open = false;

	const form = superForm(theForm, {
		validators: zodClient(newLocationsSchema),
		onUpdate(event) {
			if (event.form.valid) {
				open = false;
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="max-h-[90vh] overflow-auto">
		...
		<form use:enhance method="POST" action="?/newLocations">
