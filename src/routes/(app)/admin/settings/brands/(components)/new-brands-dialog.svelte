<script lang="ts">
	...
	import { newBrandsSchema, type NewBrandsSchema } from "../(data)/schema.js";
	...
	export let theForm: SuperValidated<Infer<NewBrandsSchema>>;

	let open = false;

	const form = superForm(theForm, {
		validators: zodClient(newBrandsSchema),
		onUpdate(event) {
			if (event.form.valid) {
				open = false;
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="max-h-[90vh] overflow-auto">
		...
		<form use:enhance method="POST" action="?/newBrands">
