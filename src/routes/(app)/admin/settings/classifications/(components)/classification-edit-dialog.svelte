<script lang="ts">
	...
	import { editClassificationSchema } from "$lib/zod-schemas/classification.schema";
	...
	export let theForm: SuperValidated<Infer<typeof editClassificationSchema>> =
		$page.data.editClassificationForm;
	...
	const form = superForm(theForm, {
		validators: zodClient(editClassificationSchema),
		id: `edit-classification-form-${classificationName}`,
		onUpdate(event) {
			if (!event.form.valid) {
				return;
			}
...
<Dialog.Root bind:open>
	...
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>Edit Classification "{classificationName}"?</Dialog.Title>
		</Dialog.Header>
		<form action="?/editClassification" method="POST" use:enhance class="space-y-4">
			<Form.Field {form} name="newName">
				<Form.Control let:attrs>
					<Form.Label>New Classification</Form.Label>
			...
			>
				<Form.Control let:attrs>
					...
					<div class="space-y-1 leading-none">
						<Form.Label>Also edit equipment in this classification</Form.Label>
					</div>
