<script lang="ts">
	...
	import { newClassificationsSchema, type NewClassificationsSchema } from "../(data)/schema.js";
	...
	export let theForm: SuperValidated<Infer<NewClassificationsSchema>>;

	let open = false;

	const form = superForm(theForm, {
		validators: zodClient(newClassificationsSchema),
		onUpdate(event) {
			if (event.form.valid) {
				open = false;
...
<Dialog.Root bind:open>
	<Dialog.Trigger class={buttonVariants()}>Add classification</Dialog.Trigger>
	<Dialog.Content class="max-h-[90vh] overflow-auto">
		<Dialog.Header>
			<Dialog.Title>Create a new Classification</Dialog.Title>
		</Dialog.Header>
		<form use:enhance method="POST" action="?/newClassifications">
