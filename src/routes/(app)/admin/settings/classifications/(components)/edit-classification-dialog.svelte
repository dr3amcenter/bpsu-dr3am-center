<script lang="ts">
	...
	import { editClassificationSchema, type EditClassificationSchema } from "../(data)/schema";
	...
	export let theForm: SuperValidated<Infer<EditClassificationSchema>> =
	...
	const form = superForm(theForm, {
		...
		id: `delete-form-${classificationName}`,
		onUpdate(event) {
			if (!event.form.valid) {
				return;
			}
